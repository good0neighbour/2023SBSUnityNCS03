...
Shader "Ryu/shDiffuseWarp_3"
{
    ...
    SubShader
    {
        ...
        float4 LightingDiffuseWarp(SurfaceOutput s, float3 lightDir, float3 viewDir, float atten)
        {
            ...
            float tSpecular = dot(s.Normal, tRyu);
            tSpecular = saturate(tSpecular);    //[0, 1]
            ...
            float4 tTexColor = tex2D(_RampTex, float2(tDot, tSpecular));

            //tResult.rgb = s.Albedo * tTexColor;//tDot;

            //ramp texture의 uv중에서 v를 임의의 기준값으로 판단하여 하일라이트 구간을 만들었다.
            if (tSpecular >= 0.95f)
            {
