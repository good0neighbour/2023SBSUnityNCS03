...
/*
	...
	'시야절두체'의 평면을 기준으로 클리핑clipping 처리
...
public:
	...
	bool OnUserUpdate(float fElapsedTime) override
	{
		...
		// <--- 요즘 렌더링 파이프라인에서는 다른 방법(z-buffer, depth byffer, 픽셀단위 <--O(1))을 사용한다.
		//<-- 삼각형의 무게중심의 z값을 기중으로 하여 내림차순 정렬하겠다. ( 화면 안쪽으로 들어갈수록 z+ 가정)
		...
		//p.s. Z-fightinng: 두 개 이상의 폴리곤이 z위치가 같은 경우 어느 폴리곤이 먼저 렌더링될지 랜덤하게 결정되므로
