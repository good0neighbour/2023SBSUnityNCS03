...
public:
	...
	bool OnUserUpdate(float fElapsedTime) override
	{
		...
		for (int ti = 0; ti < tMesh.tris.size(); ++ti)
		{
			MultiplyMatrixVector(tMeshScale.tris[ti].p[0], tMeshProj.tris[ti].p[0], tMatProj);
			MultiplyMatrixVector(tMeshScale.tris[ti].p[1], tMeshProj.tris[ti].p[1], tMatProj);
			MultiplyMatrixVector(tMeshScale.tris[ti].p[2], tMeshProj.tris[ti].p[2], tMatProj);
		}
